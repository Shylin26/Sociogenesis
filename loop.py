...
def _run_benchmark(
    ...
    for i, (task_type, task_desc, difficulty, reward) in enumerate(TASK_BANK):
...
def run(n_agents=N_AGENTS, max_ticks=TICKS, seed=42):
    ...
    log.info("=" * 55)
    log.info("PANTHEON  Week 7 — Self-Modeling + Evolution")
    log.info("=" * 55)
    ...
    for tick in range(1, max_ticks + 1):
        ...
        if tick % BENCHMARK_INTERVAL == 0:
            bscore = _run_benchmark(
                engine, cf, aggregator, router,
                episodic, librarian,
                type_seeds, fingerprints, private_latents,
                living, tick_offset=tick,
            )
            benchmark_scores.append(bscore)
            log.info(
                f"\n  [BENCHMARK @ tick {tick}] mean_quality={bscore:.4f}  "
                f"run={len(benchmark_scores)}"
            )
    ...
    log.info(f"  society model     : {society_model.snapshot()}")
    log.info(f"  historian reports : {historian.snapshot()}")
    log.info(f"  benchmark scores  : {[round(s, 4) for s in benchmark_scores]}")

    if len(benchmark_scores) >= 2:
        b1     = benchmark_scores[0]
        b_last = benchmark_scores[-1]
        improvement = (b_last - b1) / (b1 + 1e-8)
        log.info(f"  benchmark improvement: {improvement * 100:.1f}%")
        passed = (
            avg_q >= 0.75
            and stats["coalition_wins"] / max(1, stats["coalition_total"]) >= 0.60
            and stats["evolution_events"] >= 1
            and society_model.snapshot()["train_steps"] > 0
        )
    else:
        passed = avg_q >= 0.70

    log.info(f"\n  DELIVERABLE: {'PASS' if passed else 'FAIL'}")
    return stats, benchmark_scores, historian.all_reports()
